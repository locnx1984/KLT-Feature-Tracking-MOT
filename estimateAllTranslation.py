...
from estimateFeatureTranslation import estimateFeatureTranslation,estimateFeatureTranslation2
...
def estimateAllTranslation2(startXs,startYs,img1,img2): 
    ...
    for i in range(np.size(startXs)):
        if startXs_flat[i] != -1:
            ...
            newXs[i], newYs[i] = estimateFeatureTranslation2(startXs_flat[i], startYs_flat[i], Ix, Iy, img1, img2)
    newXs = np.reshape(newXs, startXs.shape)
    newYs = np.reshape(newYs, startYs.shape)
    return newXs, newYs
